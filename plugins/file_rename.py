...
def extract_season_episode(text: str):
    """Extraire la saison et l'épisode du texte"""
    patterns = [
        r'S(\d+)E(\d+)',
        r'S(\d+).*?EP?(\d+)',
...
def extract_series_name(text: str) -> str:
    """Extraire le nom de la série du texte"""
    # Supprimer les motifs saison/épisode et extensions
    patterns_to_remove = [
        r'S\d+E\d+.*',
        r'S\d+\s*EP?\s*\d+.*',
        r'Season\s*\d+.*',
        r'\.mp4$', r'\.mkv$', r'\.avi$', r'\.mov$', r'\.wmv$',
        r'\.mp3$', r'\.m4a$', r'\.flac$'
    ]
    ...
    # Nettoyer les caractères spéciaux en fin de chaîne
    series_name = re.sub(r'[\.\-\_\s]+$', '', series_name)
...
async def send_to_channel_sorted(client, file_path, caption, thumb_path, media_type, file_size, user_id, original_message):
    """Envoyer le fichier vers le canal"""
    ...
    try:
        # Déterminer le type d'envoi basé sur le type de média et la taille
        if media_type == "video":
            if file_size > 50 * 1024 * 1024:  # Si vidéo > 50MB, envoyer comme document
                message = await client.send_document(
                    ...
                    progress_args=("📤 Envoi au canal...", original_message, time.time())
                )
            else:
        ...
        await client.send_message(user_id, f"❌ Erreur lors de l'envoi au canal: {e}")
        return None
...
async def process_file(client, message, media, new_name, media_type):
    ...
    # Vérifier si l'utilisateur est autorisé à envoyer dans le canal
    user_id = message.from_user.id
    if user_id in Config.CHANNEL_ADMINS:
        # Envoyer vers le canal
        channel_msg = await send_to_channel_sorted(
            client, 
            file_path, 
            caption, 
            ph_path, 
        ...
        if channel_msg:
            await ms.edit("✅ Fichier envoyé avec succès dans le canal !")
            ...
            await ms.edit("❌ Échec de l'envoi au canal, envoi en PV...")
    
    # Si pas autorisé ou échec envoi canal, envoyer en PV
    max_retries = 3
    ...
    try:
        ...
        if episode_number:
            ...
            if user_id in Config.CHANNEL_ADMINS:
                ...
                if channel_msg:
                    await upload_msg.edit("✅ Fichier envoyé avec succès dans le canal !")
                    ...
                    # Supprimer le message de téléchargement
                    try:
                    ...
                    await upload_msg.edit("❌ Échec de l'envoi au canal, envoi en PV...")
